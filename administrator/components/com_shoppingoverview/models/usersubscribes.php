...
class ShoppingoverviewModelUsersubscribes extends JModelList
{
    ...
    {
        $form = $this->loadForm('com_shoppingoverview.usersubscribes', 'usersubscribes', array('control' => 'jform', 'load_data' => $loadData));
        if (empty($form)) {
            return false;
        }
        return $form;
    ...
    public function getListItems($filter = null,$count=null){
        ...
        $sql .= '
          WHERE
          primary_user_id LIKE '.$database->quote($filter->search).'
          OR
          user_id LIKE '.$database->quote($filter->search).'
        ';
        }
    ...
    public function save($data){
        ...
        if(isset($data->id) && $data->id != ''){
            ...
            SET
            primary_user_id = '.$database->quote($data->primary_user_id).',
            ...
            VALUES
            (
            '.$database->quote($data->primary_user_id).',
    ...
    public function validate($data){
        ...
        if(empty($data->primary_user_id)){
            $error['text'] .= "Автор не заполнен<br/>";
            $error['status'] = false;
        }

        if(empty($data->user_id)){
            $error['text'] .= "Пользователь не заполнен<br/>";
            $error['status'] = false;
        }
