...
<form id="adminFormFilter" name="adminFormFilter" method="post" action="/administrator/index.php?option=com_shoppingoverview&controller=items&task=filter">
    ...
    <div class="btn-wrapper" style="float: right;margin-left: 10px;">
        <select id="filter_published" name="filter[published]">
            <?php
            $published = array(
                '0'=>'Все',
                '1'=>'Опубликованные',
            ...
            foreach($published as $key => $value):
                ?>
                <option value="<?=$key?>" <?php if($key == $this->filter->published): ?>selected="selected"<?php endif; ?> ><?=$value?></option>
            <?php
            endforeach;
            ?>
        </select>
    ...
    <div class="btn-wrapper" style="float: right;margin-left: 10px;">
        <select id="filter_trash" name="filter[trash]">
            <?php
            $trash = array(
                '0'=>'Все',
                '1'=>'Удаленые',
            ...
            foreach($trash as $key => $value):
                ?>
                <option value="<?=$key?>" <?php if($key == $this->filter->trash): ?>selected="selected"<?php endif; ?> ><?=$value?></option>
            <?php
            endforeach;
            ?>
        </select>
