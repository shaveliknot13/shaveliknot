...
class ShoppingoverviewControllersNotifications extends JControllerLegacy
{

    function display( $cachable = false, $urlparams = array() )
    {
        JToolBarHelper::title('Уведомления');
        ...
        $filter = $session->get('AdminNotificationsFilterSearch', false);
        ...
        if($filter == false){
            ...
            $filter->limit = '10';
            $session->set('AdminNotificationsFilterSearch', $filter);
        }
    ...
    {
        JToolBarHelper::title('Новое уведомления');
        JToolBarHelper::save();
        JToolBarHelper::back();
    ...
    function save( $cachable = false, $urlparams = array() )
    {
        ...
        if(!$validate['status']){
            ...
            if($model->save($data)){
                ...
                $app->Redirect('index.php?option=com_shoppingoverview&controller=notifications');
    ...
    function  filter(){
        ...
        $session = JFactory::getSession();
        $session->set('AdminNotificationsFilterSearch', $data);
        $app->Redirect('index.php?option=com_shoppingoverview&controller=notifications');
    }
    ...
    function ajaxGetItems(){
        ...
        if($filter->limit != 0) {
            ...
            ob_start();
            require_once(dirname(dirname(__FILE__)) . '/views/notifications/tmpl/items.php');
